...
## Claude Code Best Practices 運用ルール（追加）

- 小さく安全な変更を徹底する
  - 作業は「一つの意図・小さな差分」に分割し、常に動作可能な状態を維持
  - 失敗時はすぐにロールバックできるようコミット粒度を細かく
- 変更前・変更後の最小チェックを必ず実行する
  - 型チェック: `npm run -s typecheck`
  - Lint: `npm run -s lint`（原則 `--max-warnings=0` を目安）
  - フォーマット: `npx prettier --check .`
- 自動修正の方針
  - ローカル開発では `prettier --write` / `eslint --fix` を許可
  - CI/検証フローではチェックのみ（書き換え禁止）
- 変更の進め方（推奨フロー）
  1. 現状把握（型・lintの結果/該当箇所の読み込み）
  2. 小さな設計メモ（目的・方針・影響範囲）
  3. パッチ適用（差分最小）
  4. 型/テスト/動作確認
  5. 変更要約とリスク/代替案の共有
- テスト原則
  - 不具合は「再現ケース→最小修正→回帰確認」の順で対処
  - UIはスナップショットのノイズを抑えつつ、ロジックはユニットテストを優先
- コード生成・変更の提示ルール
  - 変更部分のみを明瞭に提示（このリポジトリのルールに準拠）
- セキュリティ/シークレット
  - 機密値は環境変数で管理し、ログやコードに出さない
- PRルール
  - 小さなPR、明確な要約、リスクと代替案、テスト証跡（型/テスト/スクショ等）
- 依存関係アップデート
  - デフォルトは minor/patch を優先。major は別PRで検証計画を含めて対応
- ロールバック戦略
  - フィーチャーフラグやガードで段階的に有効化し、すぐ無効化できる設計

- コミット/プッシュ/PRの実行タイミング（重要）
  - コミット、プッシュ、PR作成、マージの実行は、ユーザーの明示的な指示がある場合に限る
  - 既定動作: 変更提案・パッチ（diff）作成までで止め、ステージング/コミット/プッシュ/PRは保留する
  - 継続作業時も、コミット粒度やメッセージはユーザー指示に従い、勝手に細分化・結合しない
  - PR マージ方式（merge/squash/rebase）もユーザー指示を待つ（デフォルトで実施しない）

### このプロジェクトでの追加ガイド

- Prettier/ESLint の実行スコープは基本的に `src/` のみ（`node_modules/` や `dist/` 等は対象外）
- 例外ハンドリングは原則 `catch {}` を用い、未使用変数警告を回避
- 本番コードの `console.log` は禁止（必要なら `__DEV__` ガード下で最小限）
- 未使用の import / 変数は必ず削除し、IDE/ESLint の自動修正を活用
- 型安全性を優先し、`any` の使用は避ける（`unknown` や具象型で置換）
